...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('admin.banners.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="col-span-2">
                ...
                <input type="url" name="image_url" value="{{ old('image_url') }}" placeholder="https://example.com/image.jpg"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                <p class="text-gray-500 text-sm mt-1">Enter a URL to an external image or upload to storage/banners and use the path below.</p>
                @error('image_url')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="col-span-2">
                ...
                <input type="text" name="image_path" value="{{ old('image_path') }}" placeholder="banners/filename.jpg"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('image_path')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="button_link" value="{{ old('button_link') }}" placeholder="#about"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('button_link')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
...
<select name="is_active" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="1" {{ old('is_active', true) == 1 ? 'selected' : '' }}>Active</option>
